...
##Network & behavioral cluster (Figure S6D)
```{r}
#network function
net_vis_z_behtype <- function(d, i, var){
  ...
  set.seed(15)
  pdf(paste0("data/Figure/Figure5/FigureS6D/", colony, ".pdf"), 
    width = 3, height = 3)
  net_ss <- graph.data.frame(d = d_syn, vertices = id_data, directed = FALSE)
  E(net_ss)$width <- d_syn %>% pull(!!sym(var)) * 0.1
...
ggsave("data/Figure/Figure5/FigureS6A.pdf", width = 3, height = 4.5, units = "in")
...
##Sex combination (Figures S6B & S6C)
```{r}
#data
...
pair_res <- pairs(emm_res, adjust = "holm") %>% as_tibble()
write.csv(pair_res, row.names = F, "data/statistical_result/FigureS6B.csv")
...
ggplot() + 
  ...
  theme(aspect.ratio = 1.65, legend.position = "none",
        axis.title = element_blank(), axis.text = element_blank())
ggsave("data/Figure/Figure5/FigureS6B.pdf", width = 2.5, height = 4, units = "in")
...
pair_res <- pairs(emm_res, adjust = "holm") %>% as_tibble()
write.csv(pair_res, row.names = F, "data/statistical_result/FigureS6C.csv")
...
ggplot() + 
  ...
  theme(aspect.ratio = 1.65, legend.position = "none",
        axis.title = element_blank(), axis.text = element_blank())
ggsave("data/Figure/Figure5/FigureS6C.pdf", width = 2.5, height = 4, units = "in")
```
...
vector_const_loc_5 <- function(k){
  ...
  for(j in 1:6){
    ...
    #30-day vector for each id
    for(name in days_5){
      d <- fread(paste0("data/processed_data/locnest_data/", colony, "/", name, "_locnest.csv")) %>% 
        mutate(Time1s = ceiling((Time-0.01)*1)) %>% 
        distinct(Time1s, .keep_all = TRUE) %>% 
        select(-Time, -Time1s)
    ...
    for(i in 1:size){
      tmp <- get(paste0("tmp_", i))
      list_name <- paste0("d_", i, "_list")
      d_list <- get(list_name)
      d_list[[j]] <- tmp
      assign(list_name, d_list, envir = .GlobalEnv)
    }
  }
}
```
...
```{r}
##synchronized times calculation function (location)
syn_loc_list <- function(k, N, ran, ID1, ID2){
  if(ran[k] == 1){
    ID1r <- ID1
  } else {
    ID1r <- ID1[c(ran[k]:N,1:(ran[k]-1))]
  }
  ID12 <- tibble(ID1r, ID2) %>% 
    mutate(Syn = case_when(ID1r == "0" & ID2 == "0" ~ 0, 
                           ID1r == ID2 ~ 1, 
...
#permutation (circular) test for rest/active function
syn_loc_pt.test_list <- function(ID1, ID2, nperm){
  #determine sequence to permute vector randomly
  N <- length(ID1)
  nperm_use <- min(nperm, N - 1) #in case N-1 < nperm
  
  set.seed(20)
  ran <- sample(2:N, nperm_use, replace = FALSE)
  ran <- c(1, ran) #add actual value to the first element
  
  DL <- 0
  for(k in 1:(nperm_use+1)){
    syn_loc_list(k, N, ran, ID1, ID2)
    DL <- c(DL, Dl)
  }
  DL <<- DL[-1]
}
...
#permutation test for rest/active performing function
syn_loc_pttest_list <- function(colony, nperm){
  size <- length(idlist[[colony]])
  DLs_all <- vector("list", 6)
  ...
  for(seg in 1:6){
    ...
    for(i in 1:(size - 1)){
      ID1_list <- get(paste0("d_", i, "_list"))
      ID1 <- ID1_list[[seg]]
      for(j in (i + 1):size){
        n <- size*(size - 1)/2 - (size - i + 1)*(size - i)/2 + (j - i)
        setTxtProgressBar(pb, n)

        ID2_list <- get(paste0("d_", j, "_list"))
        ID2 <- ID2_list[[seg]]

        d_ij <- tibble(ID1 = ID1, ID2 = ID2) %>% filter(ID1 != "N", ID2 != "N")
        ID1_use <- d_ij$ID1
        ID2_use <- d_ij$ID2

        syn_loc_pt.test_list(ID1_use, ID2_use, nperm)
        DLs[[n]] <- DL
      }
    }
    DLs_all[[seg]] <- DLs
...
for (l in 1:5){
  vector_const_loc_5(l)
  system.time(syn_loc_pttest_list(l, nperm))
  size <- idlist[[l]] %>% length()
  n_pair <- size * (size - 1) / 2

  for (seg in 1:6) {
    DL_df <- as.data.frame(do.call(cbind, DL_seg))
    DL_df <- data.frame(DL_seg)
    colnames(DL_df) <- 1:n_pair
    rownames(DL_df) <- paste0("perm_", 1:nperm)
    
    nth <- paste0("DT_", l, "_seg", seg)
    assign(nth, DT_df)
  }
}
#output data: DL_1~5_seg1~6


##Integrate data
trans_tidy_loc_seg <- function(obj_name){
  df <- get(obj_name)
  colony   <- as.integer(str_extract(obj_name, "(?<=DL_)\\d+"))
  ...
  df %>% 
    ...
    select(Colony, Seg, Comb, index, L_val)
}

df_list <- list()
for (i in 1:5) {
  for (j in 1:6) {
    obj_name <- paste0("DL_", i, "_seg", j)
    df_tidy <- trans_tidy_loc_seg(obj_name)
    df_list[[length(df_list) + 1]] <- df_tidy
  }
}
DL_t_t_l <- bind_rows(df_list)
```

##Proximity index data
```{r}
#p-value summarize function
pv_data_prox5 <- function(d){
  d %>% group_by(Comb) %>% 
    reframe(Value = Prox[index == 1],
            Med = median(Prox),
            Pn = sum(Prox >= Prox[index == 1]) / 1000) %>% 
    mutate(Sign = if_else(Pn < 0.5, "Plus", "Minus")) %>% 
...
for(i in 1:5){
  for(j in 1:6){
    d <- DL_t_t_l %>% filter(Colony == i, Seg == j) %>% mutate(Prox = L_val)
    pv <- pv_data_prox5(d) %>% mutate(Colony = i, Seg = j)
    pv0 <- pv0 %>% bind_rows(pv)
  }
}
pv_prox5 <- pv0 %>% slice(-1)


#proximity index data (z_value = proximity index)
DL_t_t_l_sim_s <- DL_t_t_l %>% 
  group_by(Colony, Seg, Comb) %>% 
  filter(index != 1) %>% 
  summarise(Mean = mean(L_val), SD = sd(L_val))
zv_prox5 <- pv_prox5 %>% 
  left_join(DL_t_t_l_sim_s, join_by(Colony, Seg, Comb)) %>% 
  mutate(z_value = (Value - Mean) / SD) %>% 
  left_join(ID_att_comb, join_by(Colony, Comb))
```
...
```{r}
strength_prox <- list()
combs <- zv_prox5 %>% distinct(Colony, Seg)

for (k in 1:nrow(combs)) {
  col <- combs$Colony[k]
  seg <- combs$Seg[k]

  df_sub <- zv_prox5 %>% filter(Colony == col, Seg == seg)
  ...
  strength_prox[[length(strength_prox) + 1]] <- result
}
...
strength_prox <- bind_rows(strength_prox) %>% 
...
#mean data
strength_prox_mean <- strength_prox %>% 
  group_by(Colony, ID) %>% 
...
##Individual differences in strength (Figure S6E)
```{r}
#plot
RdYlBu <- c("#313695", "#4575b4", "#74add1", "#abd9e9", "#e0f3f8", "#ffffbf", 
              "#fee090", "#fdae61", "#f46d43", "#d73027","#a50026")
...
for(i in 1:5){
  ...
  p <- ggplot(strength_prox %>% filter(Colony == i), 
              aes(x = ID, y = Seg, fill = yj.Strength)) + 
    geom_tile() + 
    scale_fill_gradientn(colors = colorRampPalette(RdYlBu)(n=100)) + 
    theme_void() + 
    theme(aspect.ratio = 10 / size,
          legend.position = "none", 
          axis.title = element_blank(), axis.text = element_blank())
  ggsave(plot = p, 
         paste0("data/Figure/Figure5/FigureS6E_heatmap_", colony, ".pdf"), 
...
for(i in 1:5){
  colony <- colonyname[i]
  size <- idlist[[i]] %>% length()
  p <- ggplot(strength_prox_mean %>% filter(Colony == i), 
              aes(x = ID, y = Seg, fill = mean_yj)) + 
    geom_tile() + 
    scale_fill_gradientn(colors = colorRampPalette(RdYlBu)(n=100)) + 
    theme_void() + 
    theme(aspect.ratio = 10 / size / 6, 
          legend.position = "none", 
          axis.title = element_blank(), axis.text = element_blank())
  ggsave(plot = p, 
         paste0("data/Figure/Figure5/FigureS6E_mean_heatmap_", colony, ".pdf"), 
...
for(i in 1:5){
  ...
  ggsave(plot = p, 
         paste0("data/Figure/Figure5/FigureS6E_meansdplot_", colony, ".pdf"), 
...
waic(blmm_pro_str_id) #4159.8

re_blmm_pro_str_id <- posterior_samples(blmm_pro_str_id, pars = "^r_ID\\[")
re_blmm_pro_str_id_summary <- re_blmm_pro_str_id %>%
...
write.csv(re_blmm_pro_str_id_summary, row.names = F, "data/statistical_result/FigureS6E_test.csv")
```

##Cluster differences in strength (Figure S6F)
```{r}
#zscore plot
colorlist <- c("#BE5378", "#CE7754", "#DFC44E", "#54B345", "#3B98B2", "#5F7EB2", "#808080")
ggplot(strength_prox %>% filter(is.na(Cluster) == FALSE), 
...
ggsave("data/Figure/Figure5/FigureS6F.pdf", 
       width =  4, height = 4, units = "in")


#LMM
lmm_pro_str_clu <- lmerTest::lmer(
  data = strength_prox,
  Strength ~ Cluster + (1|ID) + (1|ColSeg)
)
r2(lmm_pro_str_clu) #Conditional R2: 0.807 Marginal R2: 0.195

#EMM
emm <- emmeans(lmm_pro_str_clu, ~ Cluster)
grand_mean <- mean(summary(emm)$emmean)
test_res <- test(emm, null = grand_mean) %>% as_tibble()
write.csv(test_res, row.names = F, "data/statistical_result/FigureS6F_test.csv")
```


#5 Correlation between synchrony and proximity index (Figure 5I)
```{r}
